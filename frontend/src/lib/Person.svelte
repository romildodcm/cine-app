<script>
    ...
    async function getPersonById() {
        ...
        const res = await fetch(`${API_URL}:8001/artista/id/${person_id}`);
    ...
    async function getPersonByName() {
        ...
        const res = await fetch(`${API_URL}:8001/artista/name/${person_name}`);
        const data = await res.json();
        if (res.ok) {
            person = data;
            message = null;
...
<button on:click={getPersonById}> find </button>

<!-- By name -->
<input bind:value={person_name} type="text" placeholder="person name: Arnold" />
<button on:click={getPersonByName}> find </button>
